---
title: "AWS Batch"
description: "Install OpenSRE on AWS Batch (Multi-Node) under two minutes"
---
...
Try OpenSRE in Your AWS Batch Workloads
...
Open the following link to connect your AWS account automatically to OpenSRE. This will allow OpenSRE to monitor your AWS Batch jobs.
...
### 3. Append OpenSRE install in EC2 Launch Template UserData
...
This allows OpenSRE to correlate all jobs from the same workflow execution.
...
Update the user_data_mime.sh file to include your OpenSRE user ID. Navigate to the terraform directory and edit the file:
...
<Info>
  **Alternative for OpenSRE AWS Account Users:** If you have access to OpenSRE's
  AWS account, you can skip the infrastructure setup and run the pipeline
  directly: ``` cd nextflow-test-pipelines/pipelines/aws-batch/rnaseq
  ./run.sh tracer run ```
</Info>
